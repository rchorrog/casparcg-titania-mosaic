...
struct AVProducer::Impl
{
        ...
        while (!thread_.interruption_requested()) {
...

            {
                ...
                if (discontinuity) {
                    ...
                    frame_flush_ = true;
                    // NOTA DE LA FUSION: una reconexion real (no solo un wrap de PTS) cambia
                    // input_->start_time de verdad - a diferencia del guard-once de mas arriba
                    // (poblado solo en la primera conexion), aqui SI hay que refrescar el atomic
                    // sin condicion, o file/origin_start_time y sync/source-time quedarian
                    // desincronizados entre si tras reconectar. Seguro en este hilo (el mismo que
                    // posee input_).
                    origin_start_time_ = input_->start_time != AV_NOPTS_VALUE ? input_->start_time : 0;
                    reset(input_->start_time != AV_NOPTS_VALUE ? input_->start_time : 0);
                    frame = Frame{};
    ...
    void update_state()
    {
        ...
        // against its own `input_->start_time`.
        {
            ...
            // NOTA DE LA FUSION: el PR original leia esto via `input_.operator->()` (→
            // `ic_.get()` sin bloquear `ic_mutex_`, ver av_input.h/cpp) directamente aqui, en
            // update_state() - que corre en el hilo del mixer/canal (ver el CASPAR_SCOPE_EXIT en
            // next_frame()/prev_frame()), NO en el hilo de decodificacion que es el unico dueno
            // de `input_`. Es la misma carrera de datos que origin_start_time_ (arriba) ya evita
            // cacheando en un atomic poblado en run(); reutilizamos ese mismo atomic aqui en vez
            // de repetir el acceso inseguro.
            const auto start_time = origin_start_time_.load();
            if (src != AV_NOPTS_VALUE && start_time != AV_NOPTS_VALUE) {
